...
class RetryArenaMessageProcessorService(
	...
	private fun processMessagesWithStatus(
		status: IngestStatus,
		batchSize: Int,
	) {
		...
		try {
			when (arenaDataDbo.arenaTableName) {
				ARENA_GJENNOMFORING_TABLE_NAME ->
					gjennomforingConsumer.handleArenaMessage(
				...
				ARENA_HIST_DELTAKER_TABLE_NAME ->
					histDeltakerConsumer.handleArenaMessage(
						toArenaKafkaMessage(
							arenaDataDbo,
						),
					)
			...
			if (e is IgnoredException) {
				...
				log.error("${arenaDataDbo.id} in table ${arenaDataDbo.arenaTableName}: ${e.message}", e)
			}
